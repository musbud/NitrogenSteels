...
The following chapters provide a brief explanation of **nitrogen-alloyed stainless steels** and an illustrative procedure for **controlling their nitrogen content** during the **p**ressure-**e**lectro**s**lag-**r**emelting (**PESR**) process at [Energietechnik Essen GmbH](https://www.gmh-gruppe.de/standorte/energietechnik-essen-gmbh/). The data-based analysis of nitrogen content, dependent on individual process parameters, was conducted using R (an open-source statistical programming language).

Feel free to leverage the R-scripts provided in the [musbud/NitrogenSteels](https://github.com/musbud/NitrogenSteels) repository for your own applications.
...
cor_matrix <- cor(data.cor[, c("N", "parameter_1", "parameter_2", "parameter_3", "parameter_4", "parameter_5")], use = "pairwise.complete.obs")
...
# index <- createDataPartition(data.cor$N, p = .70, list = FALSE)
# train.data <- data.cor[index, ]
# test.data <- data.cor[-index, ]
# dim(train.data)
# dim(test.data)
...
# Plot actual vs. estimated nitrogen content
ggplot(test.data, aes(x = predict, y = N)) +
  ...
  theme_bw(base_size = 25) + annotate("text", x = 0.43, y = 0.32, label = "R^2 == 0.45", size = 8, parse = T) +
...
# remove not significant parameters or intercept
...
Similar to the MLR model, Parameter_3 is the most important process parameter for controlling the nitrogen content in the **PESR**-furnace. As this parameter increases, the nitrogen content in the molten steel also increases.
...
fig.list <- lapply(features.ranked, shap.plot.dependence, data_long = data.feature) %>% map(~.x + theme(
